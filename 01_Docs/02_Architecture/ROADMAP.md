...
| 13 | **OTA** | **PARTIAL** | OTA **partition-selection logic** is **IMPLEMENTED** and offline-tested (40/40), and the application-only flash workflow uses it. OTA **transport/runtime** (upload, reboot, rollback orchestration) does **not** exist. Stage 10 is now implemented but not hardware-tested, so OTA-A may be built but cannot be gate-passed until it is. |
