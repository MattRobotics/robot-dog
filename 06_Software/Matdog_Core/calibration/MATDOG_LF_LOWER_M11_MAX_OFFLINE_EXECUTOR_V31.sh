...
GATE_BLOB=4bcc50546c3776b8fbc62e2e08f22eae4ce5f68d
...
OUT="$ARCHIVE_ROOT/MATDOG_${PROFILE}_OFFLINE_EXECUTOR_V31_${STAMP}"
...
[[ ${1:-} == --apply ]] || {
  printf '%s\n' \
    "Usage:" \
    "  bash MATDOG_LF_LOWER_M11_MAX_OFFLINE_EXECUTOR_V31.sh --apply" \
    "" \
...
if [[ -e "$NORMACORE_WORKTREE" ]]; then
  git -C "$NORMACORE_WORKTREE" rev-parse --is-inside-work-tree >/dev/null 2>&1 ||
    fail "worktree path exists but is not a Git worktree"
  [[ "$(git -C "$NORMACORE_WORKTREE" rev-parse --is-inside-work-tree)" == true ]] ||
    fail "existing path is not a Git worktree"
