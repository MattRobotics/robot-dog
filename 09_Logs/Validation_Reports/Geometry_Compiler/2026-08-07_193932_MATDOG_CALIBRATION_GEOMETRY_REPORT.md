...
schema_version: `matdog.calibration_geometry_profile.v2`
generation_timestamp_utc: `2026-08-07T19:39:32.359689+00:00`
robot_dog_commit_sha: `344675f4107683133fdb0f1f7171b54e05e6aa4b`
robot_dog_working_tree_dirty: `True`
content_sha256: `a14dd1514642de36c59e61f71a5e116d7e8b6187cb33df8606ec1ef96d53e97c`
...
| Endpoint | Declared URDF | Compiler status | Compiler mesh contact | Compiler contact pair | Hardware delta | Compatible? | Corrective action |
|---|---:|---|---:|---|---:|---|---|
| lf_hip_max | +45.000 deg | **MODEL_INCOMPLETE** | - | - | -5.60 deg | incompatible | none automatic: real stopping mechanism is not represented in the collision STL (servo/bracket internal limit); NEEDS_HUMAN_DECISION if a mesh model of that feature should be added |
| lf_hip_min | -45.000 deg | **MODEL_INCOMPLETE** | -47.500 deg | base_link <-> lf_upper_leg_link | -2.20 deg | incompatible | none automatic: real stopping mechanism is not represented in the collision STL (servo/bracket internal limit); NEEDS_HUMAN_DECISION if a mesh model of that feature should be added |
| lf_lower_leg_max | +37.500 deg | **MODEL_INCOMPLETE** | - | - | -3.25 deg | incompatible | none automatic: real stopping mechanism is not represented in the collision STL (servo/bracket internal limit); NEEDS_HUMAN_DECISION if a mesh model of that feature should be added |
| lf_lower_leg_min | -92.000 deg | **MODEL_LIMIT_MISMATCH** | -97.957 deg | lf_foot_link <-> lf_upper_leg_link | -0.18 deg | compatible | none: mesh finding (or absence of one) and hardware evidence agree within tolerance |
| lf_upper_leg_max | +122.500 deg | **NO_MODELED_ENDSTOP** | - | - | +0.09 deg | compatible | none: mesh finding (or absence of one) and hardware evidence agree within tolerance |
| lf_upper_leg_min | -52.500 deg | **NO_MODELED_ENDSTOP** | - | - | +1.05 deg | compatible | none: mesh finding (or absence of one) and hardware evidence agree within tolerance |
...
| LF | lower_leg | min | -92.000 deg | -97.957 deg | -5.957 deg | lf_foot_link <-> lf_upper_leg_link | 0.0012 mm | **MODEL_LIMIT_MISMATCH** | - |
| LF | upper_leg | max | +122.500 deg | - | - | - | - | **NO_MODELED_ENDSTOP** | - |
| LF | upper_leg | min | -52.500 deg | - | - | - | - | **NO_MODELED_ENDSTOP** | - |
...
| lower_leg | min | -97.957 deg (MODEL_LIMIT_MISMATCH) | -98.004 deg (MODEL_LIMIT_MISMATCH) | +0.047 deg |
...
| upper_leg | min | - (NO_MODELED_ENDSTOP) | - (NO_MODELED_ENDSTOP) | same status |
| upper_leg | max | - (NO_MODELED_ENDSTOP) | - (NO_MODELED_ENDSTOP) | same status |
| lower_leg | min | -97.957 deg (MODEL_LIMIT_MISMATCH) | -98.004 deg (MODEL_LIMIT_MISMATCH) | same status |
...
- lf_hip_max: no same-leg mesh contact found in the analysis envelope, AND LF V25 hardware shows a -5.60 deg gap from the declared limit -- the real stopping mechanism (servo/bracket internal limit, not the collision STL) is not represented in this model
- lf_hip_min: same-leg mesh contact at -47.50 deg differs from the declared limit by -2.50 deg, AND LF V25 hardware shows a -2.20 deg gap from declared that this mesh contact does not explain -- the real stopping mechanism is not represented in the collision STL
- lf_lower_leg_max: no same-leg mesh contact found in the analysis envelope, AND LF V25 hardware shows a -3.25 deg gap from the declared limit -- the real stopping mechanism (servo/bracket internal limit, not the collision STL) is not represented in this model
...
- lf_upper_leg_max: declared +122.500 deg, envelope +112.500 deg..+132.500 deg
...
- lf_hip_max: MODEL_INCOMPLETE_FOR_ENDPOINT_METROLOGY -- no same-leg mesh contact found in the analysis envelope, AND LF V25 hardware shows a -5.60 deg gap from the declared limit -- the real stopping mechanism (servo/bracket internal limit, not the collision STL) is not represented in this model. NEEDS_HUMAN_DECISION: the real mechanical endstop is not represented in the current collision STL geometry.
- lf_hip_min: MODEL_INCOMPLETE_FOR_ENDPOINT_METROLOGY -- same-leg mesh contact at -47.50 deg differs from the declared limit by -2.50 deg, AND LF V25 hardware shows a -2.20 deg gap from declared that this mesh contact does not explain -- the real stopping mechanism is not represented in the collision STL. NEEDS_HUMAN_DECISION: the real mechanical endstop is not represented in the current collision STL geometry.
- lf_lower_leg_max: MODEL_INCOMPLETE_FOR_ENDPOINT_METROLOGY -- no same-leg mesh contact found in the analysis envelope, AND LF V25 hardware shows a -3.25 deg gap from the declared limit -- the real stopping mechanism (servo/bracket internal limit, not the collision STL) is not represented in this model. NEEDS_HUMAN_DECISION: the real mechanical endstop is not represented in the current collision STL geometry.
- lf_lower_leg_min: MODEL_LIMIT_MISMATCH, mesh-predicted contact differs from the declared URDF limit by -0.103970 rad, with no direct hardware evidence available to explain it; NEEDS_HUMAN_DECISION on whether to revise the URDF limit (the compiler never does this automatically).
- lf_upper_leg_max: NO_MODELED_ENDSTOP -- no mesh contact found within the analysis envelope around the declared URDF limit; that limit likely originates from a constraint this compiler does not model (e.g. internal servo/bracket range), not from these collision meshes.
- lf_upper_leg_min: NO_MODELED_ENDSTOP -- no mesh contact found within the analysis envelope around the declared URDF limit; that limit likely originates from a constraint this compiler does not model (e.g. internal servo/bracket range), not from these collision meshes.
